...
def test_server(server_url, proxy_config=None):
    """
    测试服务器连接是否有效
    ...
    try:
        ...
        # 设置超时时间为5秒
        response = requests.get(server_url, timeout=5, proxies=proxy_config)
...
def test_all_servers(servers_list):
    ...
    for server in servers_list:
        ...
        if test_server(server, proxy_config):
